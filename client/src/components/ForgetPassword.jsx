...
const EmailInputForm = ({ email, setEmail, isLoading, handleSendEmail, setStep, setMessage }) => (
...
const OtpInputForm = ({ otp, setOtp, isLoading, handleVerifyOtp, handleSendEmail, setStep, setMessage }) => (
...
const ForgetPassword = () => {
  ...
  /**
   * --- 2. VERIFY OTP Handler (Calls POST /verify-otp) ---
   */
  ...
  /**
   * --- 3. RESET PASSWORD Handler (Calls POST /reset-password) ---
   */
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Optionally, redirect to login page here after a short delay
  ...
  const getHeaderContent = () => {
      switch (step) {
          ...
          case 'otp':
              return { title: 'Enter Verification Code', subtitle: `Please enter the 6-digit code sent to **${email}**.` };
          case 'reset':
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 bg-white p-10 rounded-xl shadow-2xl">
        ...
        {message && (
            <div 
                className={`p-3 border rounded-md text-sm ${getMessageClasses(message.type)}`}
                role="alert"
            >
                {message.text}
            </div>
